(() => {
  ...
  async function boot() {
    const response = await fetch(`${SOURCE}?v=093`, { cache: 'no-store' });
    ...
    code = code
      ...
      .replaceAll('Native Live v0.9.1', 'Native Live v0.9.3')
      .replaceAll('Mesraah Native Live v0.9.1', 'Mesraah Native Live v0.9.3');

    (0, eval)(`${code}\n//# sourceURL=mesraah-native-live-v093-runtime.js`);
    window.__MESRAAH_NATIVE_LIVE_VERSION__ = '0.9.3';
  }

  boot().catch(error => {
    console.error('Mesraah Native Live v0.9.3 loader:', error);
