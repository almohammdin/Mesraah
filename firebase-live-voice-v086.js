...
// Deliberately use the normal App Check session token here.
// This matches the working text-assistant path. Limited-use tokens are only
// required when Replay Protection is enforced.
...
function installWebSocketCapture() {
  ...
  const capture = { firstMessage: '', closeCode: '', closeReason: '' };
  class DiagnosticWebSocket extends NativeWebSocket {
    constructor(url, protocols) {
      ...
      this.addEventListener('message', async event => {
        ...
        try {
          if (typeof event.data === 'string') capture.firstMessage = event.data.slice(0, 1200);
          else if (event.data instanceof Blob) capture.firstMessage = (await event.data.text()).slice(0, 1200);
        } catch {}
      }, { once: true });
  ...
  return { capture, restore: () => { window.WebSocket = NativeWebSocket; } };
}

function transportDiagnostic(capture) {
  if (!capture) return '';
  const raw = String(capture.firstMessage || '').trim();
  let first = raw ? raw.replace(/\s+/g, ' ').slice(0, 450) : 'لا توجد رسالة أولى';
  try {
    const parsed = JSON.parse(raw);
    if (parsed?.error) first = `${parsed.error.status || ''} ${parsed.error.message || ''}`.trim();
    else if (parsed?.setupComplete) first = 'setupComplete';
  } catch {}
...
async function prepare() {
  ...
  lastTransport = null;
  setGoVisible(false);
  setStatus('أجهز الاتصال الصوتي…', 'connecting', 'أستخدم نفس حماية App Check التي يعمل بها مساعد مسراح النصي.');
  ...
  try {
    ...
    setStatus('جاهز للمحادثة', 'ready', 'الاتصال الصوتي ناجح. اضغط «ابدأ المحادثة».');
    setGoVisible(true);
    ...
    setStatus(
      ...
      'error',
      `App Check: session token | ${transport} | SDK: ${diagnostic(error)}`
    );
...
async function startNow() {
  ...
  try {
    ...
    setStatus(friendlyError(error, 'audio'), 'error', `SDK: ${diagnostic(error)}`);
