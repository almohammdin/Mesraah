...
if(!window.__MESRAAH_ASSISTANT_FIRST_V0151__){
 ...
 function ensureInlineUi(){const card=$('#v11VoiceCard');if(!card||card.dataset.v15Inline)return false;card.dataset.v15Inline='1';card.classList.add('v14-assistant-home');const copy=card.querySelector('.v11-voice-copy');if(copy){const h2=copy.querySelector('h2');if(h2)h2.textContent='ماذا تريد أن تنجز اليوم؟';const p=copy.querySelector('p');if(p)p.textContent='اكتب أو تحدث بشكل طبيعي، ومسراح يوجهك إلى المهمة أو الشخص أو المساحة المناسبة.'}
  const action=card.querySelector('.v11-voice-action');const compose=document.createElement('div');compose.className='v14-inline-compose';compose.innerHTML='<input id="v14AssistantInput" maxlength="300" autocomplete="off" placeholder="مثال: رتب مهامي اليوم حسب الأولوية" aria-label="اكتب طلبك لمسراح"><button type="button" id="v14AssistantSend">إرسال</button>';const voice=action?.querySelector('#v112VoiceStart');if(voice){voice.className='v14-voice-button';voice.innerHTML='<span class="v14-voice-visual" aria-hidden="true"><svg viewBox="0 0 48 28" focusable="false"><defs><linearGradient id="v14GeminiGradient" x1="16" y1="5" x2="33" y2="23" gradientUnits="userSpaceOnUse"><stop stop-color="#4285F4"/><stop offset=".46" stop-color="#8B5CF6"/><stop offset="1" stop-color="#F06B8D"/></linearGradient></defs><g class="v14-voice-waves" fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2.2"><path d="M4 12v4M9 9v10M14 6v16M34 6v16M39 9v10M44 12v4"/></g><path d="M24 5C24.8 10 28 13.2 33 14 28 14.8 24.8 18 24 23 23.2 18 20 14.8 15 14 20 13.2 23.2 10 24 5Z" fill="url(#v14GeminiGradient)"/></svg></span><span class="v14-voice-label">تحدث</span><strong class="sr-only">بدء محادثة صوتية مع مسراح</strong>';voice.setAttribute('aria-label','بدء محادثة صوتية مع مسراح');voice.title='محادثة صوتية مع مسراح';compose.appendChild(voice)}(action||copy)?.insertAdjacentElement('afterend',compose);action?.remove();const sug=document.createElement('div');sug.id='v14Suggestions';sug.className='v14-inline-suggestions';compose.insertAdjacentElement('afterend',sug);bind();renderSuggestions(true);demotePriority();return true}
