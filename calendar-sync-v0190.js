...
function eventFingerprint(event = {}) {
  ...
  return title && when.date ? `${title}|${when.date}|${when.time}` : '';
}
...
function isMesraahEvent(event = {}) {
  return event.extendedProperties?.private?.mesraahSource === 'mesraah';
}
...
function matchingEvent(task, events = []) {
  const fp = taskFingerprint(task);
  if (!fp) return null;
  const matches = events.filter(event =>
    event?.id && event.status !== 'cancelled' && eventFingerprint(event) === fp
  );
  return matches.find(event => !isMesraahEvent(event)) || matches[0] || null;
}

function matchingCachedEvent(task) {
  return matchingEvent(task, window.MesraahCalendar?.getCachedEvents?.() || []);
}
...
async function pushTaskOnce(task) {
  ...
  if (!task.calendarEventId) {
    const matched = matchingCachedEvent(task);
    if (matched) {
      updateTaskCalendarMeta(task.id, {
        calendarEventId: matched.id,
        calendarSource: isMesraahEvent(matched) ? 'mesraah' : 'google',
        calendarDirty: false,
        calendarSyncedAt: new Date().toISOString(),
        calendarEventUpdated: matched.updated || '',
        calendarHtmlLink: matched.htmlLink || ''
      });
      return { ok: true, event: matched, linked: true };
    }
  }
  ...
  if (task.calendarEventId) {
    try {
      ...
      const matched = matchingCachedEvent(task);
      event = matched || await api.createEvent(payload);
    }
  } else {
  ...
  updateTaskCalendarMeta(task.id, {
    ...
    calendarSource: isMesraahEvent(event) ? 'mesraah' : (task.calendarSource || 'google'),
    calendarDirty: false,
    calendarSyncedAt: new Date().toISOString(),
...
function eventToTaskFields(event) {
  ...
  return {
    ...
    calendarSource: isMesraahEvent(event) ? 'mesraah' : 'google',
    calendarDirty: false,
    calendarSyncedAt: new Date().toISOString(),
    calendarEventUpdated: event.updated || '',
...
function mergeGeneratedDuplicates(state) {
  ...
  for (const group of groups.values()) {
    const originals = group.filter(task => !isImportedTask(task));
    const imports = group.filter(isImportedTask);
    if (originals.length !== 1 || !imports.length) continue;
    const canonical = originals[0];
    const best = imports.find(task => task.calendarSource === 'google' && task.calendarEventId)
      || imports.find(task => task.calendarEventId)
      || imports[0];
    if (!canonical.calendarEventId && best?.calendarEventId) {
      CAL_FIELDS.forEach(field => {
        if (best[field] !== undefined) canonical[field] = best[field];
      });
    }
...
function pullEvents(events, dirtyIds) {
  const state = readState();
  state.tasks = Array.isArray(state.tasks) ? state.tasks : [];
  let merged = mergeGeneratedDuplicates(state);
  let imported = 0;
  let updated = 0;
  let changed = merged > 0;
  const duplicateEventIds = new Set();

  const byId = new Map(state.tasks.map(task => [String(task.id), task]));
  const byEvent = new Map(
    state.tasks.filter(task => task.calendarEventId).map(task => [String(task.calendarEventId), task])
  );
  const ordered = [...(events || [])]
    .filter(event => event?.id && event.status !== 'cancelled')
    .sort((a, b) => Number(isMesraahEvent(a)) - Number(isMesraahEvent(b)));
  ...
  for (const event of ordered) {
    ...
    if (!task && fp) {
      task = state.tasks.find(item =>
        !item.calendarEventId && !item.demo && !isImportedTask(item) && taskFingerprint(item) === fp
      ) || null;
    }

    if (
      task && task.calendarEventId && String(task.calendarEventId) !== String(event.id)
      && isMesraahEvent(event) && fp && taskFingerprint(task) === fp
    ) {
      duplicateEventIds.add(String(event.id));
      continue;
    }
    ...
    if (task && dirtyIds.has(String(task.id))) {
      if (!task.calendarEventId) {
        ...
        task.calendarSource = isMesraahEvent(event) ? 'mesraah' : 'google';
        task.calendarEventUpdated = event.updated || '';
        task.calendarHtmlLink = event.htmlLink || '';
        changed = true;
    ...
    if (task) {
      ...
      const relevant = [
        'title', 'notes', 'due', 'time', 'location', 'calendarEventId',
        'calendarSource', 'calendarEventUpdated', 'calendarHtmlLink'
      ];
  ...
  const mergedAfter = mergeGeneratedDuplicates(state);
  if (mergedAfter) {
    merged += mergedAfter;
    changed = true;
  }
  if (changed) writeState(state);
  return { imported, updated, merged, changed, duplicateEventIds: [...duplicateEventIds] };
}

async function processTombstones() {
  const state = readState();
  const tombstones = Array.isArray(state.calendarTombstones)
    ? [...new Set(state.calendarTombstones.filter(Boolean))]
    : [];
...
async function deleteDuplicateEvents(ids = []) {
  let deleted = 0;
  for (const eventId of [...new Set(ids.filter(Boolean))]) {
    try {
      await window.MesraahCalendar.deleteEvent(eventId);
      deleted += 1;
    } catch (error) {
      if (Number(error?.status) === 404) deleted += 1;
      else console.warn('Mesraah duplicate calendar cleanup:', error);
    }
  }
  return deleted;
}
...
async function syncNow() {
  ...
  try {
    let deleted = await processTombstones();
    const before = readState();
    const dirtyIds = new Set(
      (before.tasks || []).filter(task => task.calendarDirty).map(task => String(task.id))
    );
    ...
    deleted += await deleteDuplicateEvents(pulled.duplicateEventIds);

    const current = readState();
    const toPush = (current.tasks || []).filter(task =>
      taskNeedsCalendar(task) && (task.calendarDirty || !task.calendarEventId)
    );
    let pushed = 0;
    let linked = 0;
    ...
    const changed = Boolean(pulled.changed || deleted || pushed || linked);
    const summary = {
      ok: true,
    ...
    if (changed) {
      ...
      try { await window.MesraahCloudBridge?.saveNow?.(); } catch {}
    }
    return summary;
  } finally {
    syncing = false;
...
function queueDeletion(task) {
  ...
  setTimeout(async () => {
    ...
    if (connected()) {
      try {
        await window.MesraahCalendar.deleteEvent(task.calendarEventId);
        return;
      } catch (error) {
    ...
    fresh.calendarTombstones = [...new Set([
      ...(fresh.calendarTombstones || []), task.calendarEventId
    ])];
    writeState(fresh);
  }, 140);
}
...
function installTaskHooks() {
  document.addEventListener('submit', event => {
    ...
    const oldTask = idBefore
      ? (before.tasks || []).find(task => String(task.id) === String(idBefore))
      : null;
    ...
    setTimeout(() => {
      const state = readState();
      let task = idBefore
        ? (state.tasks || []).find(item => String(item.id) === String(idBefore))
        : null;
      if (!task) {
        task = [...(state.tasks || [])].reverse().find(item =>
          !oldIds.has(String(item.id)) && (!title || item.title === title)
        );
      }
      ...
      if (!task.due && oldTask?.calendarEventId) {
        ...
        task.calendarDirty = false;
        task.calendarSyncedAt = new Date().toISOString();
        state.calendarTombstones = [...new Set([
          ...(state.calendarTombstones || []), oldTask.calendarEventId
        ])];
        writeState(state);
...
function installCalendarUi() {
  const findCard = () => [...document.querySelectorAll('.connection-card')]
    .find(card => card.textContent.includes('Google Calendar') || card.textContent.includes('التقويم'));
  ...
  const showSummary = summary => {
    ...
    const fixed = Number(summary.merged || 0) + Number(summary.linked || 0) + Number(summary.deleted || 0);
    if (!fromGoogle && !toGoogle && !fixed) {
      status.textContent = 'المواعيد متزامنة بين مسراح وGoogle Calendar';
    } else {
      status.textContent = `Google ← ${toGoogle} من مسراح · مسراح ← ${fromGoogle} من Google${fixed ? ` · عولج التكرار ${fixed}` : ''}`;
    }
  };
