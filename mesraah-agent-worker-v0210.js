...
function normalizeAgent(agent={}){return {version:AGENT_VERSION,enabled:agent.enabled!==false,settings:{...DEFAULT_SETTINGS,...(agent.settings||{})},inbox:Array.isArray(agent.inbox)?agent.inbox:[],log:Array.isArray(agent.log)?agent.log:[],dismissedKeys:Array.isArray(agent.dismissedKeys)?agent.dismissedKeys:[],lastRunAt:agent.lastRunAt||'',lastRunReason:agent.lastRunReason||'',lastResult:agent.lastResult||null}}
...
export function runAgent({reason='scheduled',force=false}={}){
  ...
  const nextAgent={...agent,inbox:[...added,...agent.inbox].slice(0,MAX_INBOX_ITEMS),log:[{id:uid(),at:now,reason,result},...agent.log].slice(0,MAX_LOG_ITEMS),lastRunAt:now,lastRunReason:reason,lastResult:result};
...
function setEnabled(enabled){const next=updateAgent(agent=>({...agent,enabled:Boolean(enabled)}));if(next.enabled){runAgent({reason:'enabled',force:true});showToast('بدأ وكيل مسراح المتابعة')}else showToast('تم إيقاف وكيل مسراح')}
function setRule(rule,enabled){updateAgent(agent=>({...agent,settings:{...agent.settings,[rule]:Boolean(enabled)}}));if(enabled)runAgent({reason:'settings',force:true})}
...
function boot(){
  bindUi();renderAgent();setTimeout(()=>runAgent({reason:'startup'}),900);setInterval(()=>runAgent({reason:'interval'}),RUN_INTERVAL_MS);
  let mutationTimer=0;const scheduleMutationRun=()=>{clearTimeout(mutationTimer);mutationTimer=setTimeout(()=>runAgent({reason:'task-change'}),700)};window.addEventListener('mesraah:task-mutated',scheduleMutationRun);window.addEventListener('mesraah:data-changed',scheduleMutationRun);
