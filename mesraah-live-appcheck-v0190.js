...
const APP_NAME='mesraah-live-0190';
...
window.MesraahLiveFirebaseApp=app;
...
window.MesraahLiveGetAppCheckToken=async({forceRefresh=false}={})=>{const result=await getToken(appCheck,forceRefresh);if(!result?.token)throw new Error('voice-app-check-token-missing');return result.token};
window.MESRAAH_LIVE_TOKEN_ENDPOINT='https://mesraah-live-token.naif123456.workers.dev/token';
