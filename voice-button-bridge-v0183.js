(()=>{
  if(window.__MESRAAH_VOICE_BUTTON_BRIDGE_0200__)return;
  window.__MESRAAH_VOICE_BUTTON_BRIDGE_0200__=true;
  const style=document.createElement('style');
  style.id='mesraahVoiceCta0200';
  ...
  async function ensureVoice(){
    if(window.MesraahVoice?.mode==='gemini-live-agent-0200'&&window.MesraahVoice?.start)return window.MesraahVoice;
    ...
    if(voice?.mode!=='gemini-live-agent-0200'||!voice?.start){
