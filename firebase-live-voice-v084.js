...
async function connectLiveSession() {
  ...
  finalError.mesraahAttempts = failures.map(item => ({ model: item.modelName, diagnostic: diagnostic(item.error) }));
  throw finalError;
}
...
async function prepare() {
  ...
  try {
    ...
    setStatus('جاهز للمحادثة', 'ready', 'الاتصال ناجح. اضغط «ابدأ المحادثة» لتشغيل المايك والتحدث مع مسراح.');
    setGoVisible(true);
    ...
    const attempts = Array.isArray(error?.mesraahAttempts)
      ...
      : diagnostic(error);
    setStatus(friendlyError(error, 'connect'), 'error', `رمز التشخيص: ${attempts}`);
