(()=>{
  if(window.__MESRAAH_VOICE_BUTTON_BRIDGE_0201__)return;
  window.__MESRAAH_VOICE_BUTTON_BRIDGE_0201__=true;
  const style=document.createElement('style');
  style.id='mesraahVoiceCta0201';
  ...
  async function ensureVoice(){
    if(window.MesraahVoice?.mode==='gemini-live-agent-0201'&&window.MesraahVoice?.start)return window.MesraahVoice;
    if(!window.MesraahVoiceReady){
      window.MesraahVoiceReady=(async()=>{
        if(!window.MesraahAgentBridge)await import('./mesraah-agent-bridge-v0200.js?v=0.20.1');
    ...
    if(voice?.mode!=='gemini-live-agent-0201'||!voice?.start){
