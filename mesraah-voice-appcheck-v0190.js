...
window.MesraahVoiceGetAppCheckToken=async({forceRefresh=false}={})=>{const result=await getToken(appCheck,forceRefresh);if(!result?.token)throw new Error('voice-app-check-token-missing');return result.token};
