(() => {
  ...
  function weekday(date = new Date()) {
    return clean(new Intl.DateTimeFormat('ar-SA-u-ca-gregory-nu-latn', {
      timeZone: 'Asia/Riyadh', weekday: 'long'
    }).format(date));
  }

  function hijri(date = new Date()) {
    return clean(new Intl.DateTimeFormat('ar-SA-u-ca-islamic-umalqura-nu-latn', {
      timeZone: 'Asia/Riyadh', day: 'numeric', month: 'long', year: 'numeric'
    }).format(date));
  }

  function gregorian(date = new Date()) {
    return clean(new Intl.DateTimeFormat('ar-SA-u-ca-gregory-nu-latn', {
      timeZone: 'Asia/Riyadh', day: 'numeric', month: 'long', year: 'numeric'
    }).format(date));
  }
  ...
  function buildMoment() {
    const host = document.getElementById('todayMoment');
    if (!host) return;
    host.className = 'today-moment v72-moment';
    host.innerHTML = `
      <div class="v72-time-pane">
        <span class="v72-live-label"><span class="v72-live-dot" aria-hidden="true"></span>الآن</span>
        <button class="time-toggle v72-clock" id="timeToggle" type="button" aria-label="تبديل نظام الساعة">
          <span class="v72-hm" id="todayTime"></span><span class="v72-sec" id="todaySeconds"></span><span class="v72-period" id="todayPeriod"></span>
        </button>
      </div>
      <div class="v72-date-pane">
        <div class="v72-dayline"><strong id="todayWeekday"></strong><span>اليوم</span></div>
        <div class="v72-date-row"><span class="v72-date-label">هجري</span><span class="v72-date-value" id="todayHijri"></span></div>
        <div class="v72-date-row"><span class="v72-date-label">ميلادي</span><span class="v72-date-value" id="todayGregorian"></span></div>
      </div>
      <span class="eyebrow legacy-date" id="todayDate" hidden></span>`;

    document.getElementById('timeToggle')?.addEventListener('click', () => {
      const is24 = localStorage.getItem(CLOCK_KEY) === '1';
      localStorage.setItem(CLOCK_KEY, is24 ? '0' : '1');
      renderMoment();
    });
  }
  ...
  function renderMoment() {
    ...
    const seconds = document.getElementById('todaySeconds');
    const period = document.getElementById('todayPeriod');
    const day = document.getElementById('todayWeekday');
    const h = document.getElementById('todayHijri');
    ...
    const now = new Date();
    const p = parts(now);
    const hour24 = Number(p.hour || 0);
    const is24 = localStorage.getItem(CLOCK_KEY) === '1';
    time.textContent = is24
      ? `${String(hour24).padStart(2, '0')}:${p.minute}`
      : `${hour24 % 12 || 12}:${p.minute}`;
    seconds.textContent = `:${p.second}`;
    period.textContent = is24 ? '' : (hour24 < 12 ? 'ص' : 'م');
  ...
  function installStoryIcon() {
    ...
    icon.innerHTML = `
      <svg viewBox="0 0 24 24" aria-hidden="true">
        ...
        <path d="M12 2.5v1"></path><path d="m4.8 5.2.8.8"></path><path d="m19.2 5.2-.8.8"></path>
      </svg>`;
    const glowKey = `mesraah_story_glow_${VERSION}`;
    if (!sessionStorage.getItem(glowKey)) {
      sessionStorage.setItem(glowKey, '1');
      card.classList.add('v72-glow');
      setTimeout(() => card.classList.remove('v72-glow'), 1800);
    }
  }
  ...
  function boot() {
    ...
    installStoryIcon();
    normalizeVersion();
    window.setInterval(renderMoment, 1000);
  }
