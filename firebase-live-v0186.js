...
let session=null,controller=null,active=false;
...
function cleanSchema(schema){if(!schema||typeof schema!=='object')return schema;const out={};for(const key of ['type','description','format','nullable','enum','required'])if(schema[key]!==undefined)out[key]=schema[key];if(schema.properties)out.properties=Object.fromEntries(Object.entries(schema.properties).map(([k,v])=>[k,cleanSchema(v)]));if(schema.items)out.items=cleanSchema(schema.items);return out}
const FIREBASE_TASK_TOOLS=TASK_TOOL_DECLARATIONS.map(tool=>({name:tool.name,description:tool.description,parameters:cleanSchema(tool.parametersJsonSchema||tool.parameters||{type:'object',properties:{}})}));

function instruction(){const s=readState();return `أنت مسراح، مساعد شخصي سعودي صوتي. تكلم باختصار وبلهجة سعودية سهلة.
مسراح نفسه هو مصدر الحقيقة والذاكرة الأساسية. حالة مسراح الكاملة المرفقة أدناه هي سياقك عند بدء الجلسة، ولا تعتمد على ذاكرة المحادثة وحدها.
عند أي سؤال عن المهام الحالية أو بعد أي تغيير خلال الجلسة استخدم search_tasks للحصول على الحالة الأحدث. عند الإضافة استخدم add_task، وعند التعديل ابحث أولا ثم update_task، وعند الحذف ابحث أولا ثم delete_task، وعند الإنجاز استخدم complete_task.
...
حالة مسراح الكاملة الحالية: ${JSON.stringify(s)}`}

async function executeOne(call){let result;try{result=await executeTaskTool(call.name,call.args||{});if(result?.ok&&call.name!=='search_tasks'){const cloud=await window.MesraahCloudBridge?.saveNow?.();if(cloud?.ok===false)result={ok:false,error:'cloud-save-not-confirmed'}}}catch(error){result={ok:false,error:String(error?.message||error)}}return result||{ok:false,error:'empty-result'}}
async function runTools(functionCalls=[]){const calls=Array.isArray(functionCalls)?functionCalls:[];if(!calls.length)return {name:'mesraah',response:{ok:false,error:'no-function-calls'}};const results=[];for(const call of calls){setStatus('أنفذ طلبك…','connecting');const result=await executeOne(call);results.push({name:call.name,result});setDetail(result?.ok?'تم تنفيذ طلبك في مسراح.':'تعذر إكمال التنفيذ في مسراح.')}const last=results.at(-1);return {name:last.name,response:{...last.result,batchResults:results.length>1?results:undefined}}}
