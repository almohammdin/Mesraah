...
if(!window.__MESRAAH_ASSISTANT_FIRST_V0141__){
 ...
 function openComposerWith(text){const textStart=$('#v112TextStart'),chat=$('#v112TextChat');if(chat?.hidden)textStart?.click();const input=$('#v112ChatInput');if(input){input.value=text;input.focus();$('#v112ChatSend')?.click()}}
 ...
 function mountExistingChat(){const mount=$('#v14ChatMount'),chat=$('#v112TextChat');if(mount&&chat&&chat.parentElement!==mount)mount.appendChild(chat)}
 ...
 function boot(){styles();ensureHome();setTimeout(()=>{mountExistingChat();demotePriority();renderSuggestions(true)},250);document.documentElement.dataset.mesraahVersion=VERSION}
